...
class TemporalMemory():
    ...
    def __init__(self, spatial_pooler, anomaly_tracker = None, num_cells = 16, stimulus_thresh = 13, initial_perm = 0.55, perm_thresh = 0.5, min_learning_thresh = 10, max_new_synapse = 20, perm_increment = 0.1, perm_decrement = 0.1, incorrect_pred_dec = 0.0, max_segments = 128, max_synapse_per_segment = 40, subthreshold_learning = False, ID = 'TM1'):
        ...
        #This is a variable I've been using to track time-expensive processes
        self.pred_time = 0
                    ...
                    #For each corresponding cell:
                        ...
                        if (not predictive_exists):
                            ...
                            if (learning) and (np.sum(self.last_active_cells) > 0):
                                ...
                                self.cells[cell_index].add_segment.remote(self.last_active_cells)
        ...
        for i in range(self.column_num):
            ...
            if all_cells_allowed:
                ...
                for cell in self.cells[self.num_cells*(i):self.num_cells*(i+1)]:
                    ...
                    score = cell.get_max_overlap_score_par.remote(self.active_cells)
            ...
            #If only a certain number of cells are allowed to activate per column:
                ...
                max_overlaps = [cell.get_max_overlap_score_par.remote(self.active_cells) for cell in self.cells[self.num_cells*(i):self.num_cells*(i+1)]]
                N_highest = find_N_highest(max_overlaps,num_predictive_allowed)
                N_highest_overlap = N_highest[-1]
                
                #Record the subthreshold learning and predictive cells:
                for cell_idx, overlap in enumerate(max_overlaps):
                    #Predictive cells:
                    if (overlap >= N_highest_overlap) and (overlap >= self.stimulus_thresh):
                        self.predictive_cells[cell_idx] = 1
                    #Subthreshold learning cells:
                    elif (self.subthreshold_learning) and (overlap >= self.min_learning_thresh):
                        self.subthreshold_cells[cell_idx] = 1
                
        return self.predictive_cells
    
    def find_predictive_cells_single_column(self, col_index):
        #Singleton version of the find_predictive_cells() method, used for multiprocessing.
        #Assumes no limit on # of allowed active cells per column. (I.E. all_cells_allowed = True)
        #Also does not provide for subthreshold learning.
        cells = [0]*self.num_cells
        col_bit = self.active_cols[col_index]
        if col_bit:
            for index in range(self.num_cells*(col_index),self.num_cells*(col_index+1)):
                #Get the overlap scores
                score = self.cells[index].get_max_overlap_score_par(self.active_cells)
                
                if score >= self.stimulus_thresh:
                    cells[index-self.num_cells*(col_index)] = 1
        return cells
    
    def find_predictive_cells_multiprocessing(self, pool):
        #Opens a parallel pool and maps find_predictive_cells_single_column() to it.
        #Doesn't allow for the full range of options that find_predictive_cells() does,
        #such as subthreshold learning.
        cells_list = pool.map(self.find_predictive_cells_single_column,list(range(self.column_num)))
        self.active_cells = np.array([bit for sublist in cells_list for bit in sublist]).reshape((-1,))
        return self.predictive_cells
    
    def process_input(self, SDR, tm_learning = True, sp_learning = False, new_sp_cycle = False, all_cells_allowed = True, num_allowed = -1, multi_inputs = False, boosting = False, static_sp = True, sparse_output = False, pool = None):
        ...
        if tm_learning:
            for i in range(self.num_cells*self.column_num):
                ...
                #If the cell is not active but was predictive, lower its permanences
                        ...
                        self.cells[i].update_perms.remote(correct_prediction = False)
        ...
        if pool is None:
            ...
            print("Finding predictive cells...")
            self.find_predictive_cells_multiprocessing(pool);
        ...
        if tm_learning and self.subthreshold_learning:
            for i in range(self.num_cells*self.column_num):
                if self.subthreshold_cells[i] > 0:
                    ...
                    self.cells[i].update_perms.remote(self.active_cells,correct_prediction = True)
